...
namespace Titanium.Web.Proxy.Examples.Basic
{
    ...
    public class ProxyTestController
    {
        ...
        {
            SessionEventArgs session = (SessionEventArgs)sender;
        ...
        private async Task OnResponse(object sender, SessionEventArgs e)
        {
            ...
            bool hasAuthHeader = e.WebSession.Response.Headers.HeaderExists("WWW-Authenticate");
            if (hasAuthHeader)
            {
                // System.Console.WriteLine(hasAuthHeader);
                // Titanium.Web.Proxy.Models.HttpHeader h = e.WebSession.Response.Headers.Headers["WWW-Authenticate"];
                if (e.WebSession.Response.Headers.NonUniqueHeaders.ContainsKey("WWW-Authenticate"))
                {
                    List<HttpHeader> a = e.WebSession.Response.Headers.NonUniqueHeaders["WWW-Authenticate"];
                    int ind = a.FindIndex(x => x.Value.IndexOf("negotiate", System.StringComparison.InvariantCultureIgnoreCase) != -1);

                    if (ind != -1)
                        a.RemoveAt(ind);

                    // ind = a.FindIndex(x => x.Value.IndexOf("ntlm", System.StringComparison.InvariantCultureIgnoreCase) != -1);
                    // if (ind != -1) a.RemoveAt(ind);

                    // System.Console.WriteLine(a);
                } // End if (e.WebSession.Response.Headers.NonUniqueHeaders.ContainsKey("WWW-Authenticate")) 

            } // End if (hasAuthHeader) 
